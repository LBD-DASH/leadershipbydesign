...
import { diagnosticQuestions, categories } from "@/data/diagnosticQuestions";
...
export default function DiagnosticForm({ onSubmit, isSubmitting }: DiagnosticFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-12">
      {categories.map((category) => {
        const categoryQuestions = diagnosticQuestions.filter((q) => q.category === category.key);
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="text-center sm:text-left">
            <p className="text-sm text-muted-foreground">
              {answeredCount} of {diagnosticQuestions.length} questions answered
            </p>
