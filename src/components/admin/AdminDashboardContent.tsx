...
import { LogOut, Users, Target, TrendingUp, BarChart3 } from 'lucide-react';
...
export default function AdminDashboardContent({ onLogout }: AdminDashboardContentProps) {
  ...
  const isLoading = loadingLeadership || loadingTeam;

  // Calculate metrics
  const totalLeadership = leadershipSubmissions?.length || 0;
  const totalTeam = teamSubmissions?.length || 0;
  const totalSubmissions = totalLeadership + totalTeam;
  ...
  const totalWithEmail = leadershipWithEmail + teamWithEmail;
  ...
  const totalWaitingList = leadershipWaitingList + teamWaitingList;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent>
            <p className="text-xs text-muted-foreground">
              {totalLeadership} Leadership · {totalTeam} Team
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-xs text-muted-foreground">
              {leadershipWithEmail} Leadership · {teamWithEmail} Team
            </p>
          </CardContent>
        </Card>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList>
          ...
          <TabsTrigger value="leadership">Leadership Submissions</TabsTrigger>
          <TabsTrigger value="team">Team Submissions</TabsTrigger>
        </TabsList>

        <TabsContent value="overview" className="mt-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <UTMBreakdownChart
              title="Leadership Diagnostic - UTM Sources"
              submissions={leadershipSubmissions || []}
              isLoading={loadingLeadership}
            />
            <UTMBreakdownChart
              title="Team Diagnostic - UTM Sources"
